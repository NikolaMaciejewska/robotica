...
from main import get_state, compute_reward
import robotica
...
def train_navigation_phase(obstacle_mode=True, ball_mode=True):
    ...
    epsilon = EPSILON_START_2
    step_count = 0
    episode_rewards = []

    # Hindernisobjekte
    obstacle_names = ["Obstacle1", "Obstacle2", "Obstacle3", "Obstacle4", "Obstacle5", "Obstacle6"]
    area_bounds = [-2.0, 2.0, -2.0, 2.0]  # xmin, xmax, ymin, ymax
    try:
        for episode in range(1, MAX_EPISODES + 1):
            coppelia.randomize_obstacles_positions('/Obstacles', (-2, 2), (-2, 2))
            ...
            for step in range(1, MAX_STEPS_PER_EPISODE + 1):
                ...
                if done:
                    ...
                    epsilon = max(EPSILON_END_2, epsilon * EPSILON_DECAY_2)
                    ...
                    if len(episode_rewards) >= REWARD_WINDOW_SIZE:
                        ...
                        if avg_reward >= TARGET_REWARD:
                            print("Ziel erreicht – Training frühzeitig beendet.")
                            break
                    break

    finally:
        print("Training wird beendet – Speichere Modell und Rewards.")
...
if __name__ == '__main__':
    train_navigation_phase(obstacle_mode=True, ball_mode=True)